...
+ Getting way too much information with `sinfo`
...
2. Let's use `srun` to run `samtools`. Notice the command is identical to just running `samtools sort` but we just put an `srun` in front of it. Take a note of the job id that srun returns.

```
srun /opt/installed/samtools-1.6/bin/samtools sort SRR1576820_0001.bam -o SRR1576820_0001.sorted.bam
```
...
```
srun /opt/installed/samtools-1.6/bin/samtools index SRR1576820_0001.sorted.bam
```

4. Let's confirm that we sorted the file. 

5. Let's look at statistics. We pipe (`|`) `samtools stats` into `grep` so we can just see the summary statistics.

```
srun /opt/installed/samtools-1.6/bin/samtools stats SRR1576820_0001.sorted.bam | grep ^SN | cut -f 2- 
```

6. Confirm that you have sorted the file using `samtools view` piped into `head`:

```
srun /opt/installed/samtools-1.6/bin/samtools view SRR1576820_0001.sorted.bam | head
```
...
Ok, let's look at an alternative way to do this. We may have to actually test an entire pipeline on exacloud and make sure it works, and that our script is finding the software in the entire pipeline. We'll only request one CPU with 1 Gb of memory to do our job.
...
```
/opt/installed/samtools-1.6/bin/samtools sort SRR1576820_0002.bam -o SRR1576820_0002.sorted.bam
/opt/installed/samtools-1.6/bin/samtools index SRR1576820_0001.sorted.bam
/opt/installed/samtools-1.6/bin/samtools stats SRR1576820_0001.sorted.bam | grep ^SN | cut -f 2-
```
...
Look at the statement with `arrayfile`. We'll go over this slowly so everyone understands it. Notice we haven't specified an `--array` argument. We can pass that in to `sbatch` when we run it. 
...
Also, your scripts can build in dependencies into the files. 
...
No need to worry! That's what `scancel` is for. 
